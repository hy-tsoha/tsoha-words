...
def get_full_stats(user_id):
    sql = "SELECT id, name FROM decks WHERE creator_id=:user_id"
    decks = db.session.execute(sql, {"user_id": user_id}).fetchall()
    data = []
    for deck in decks:
        sql = "SELECT u.name, COUNT(*), COALESCE(SUM(a.result),0) FROM answers a, cards c, users u WHERE c.deck_id=:deck_id AND a.card_id=c.id AND u.id=a.user_id GROUP BY u.id, u.name"
